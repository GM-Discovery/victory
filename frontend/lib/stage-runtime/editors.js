(function (root, factory) {
  ...
  root.VictoryStageEditors = factory();
  ...
  function createEditorControllers(deps = {}) {
    ...
    function hideMapEditorController() {
      ...
      setMapEditorStatus("Choose a map asset for the stage.");
    ...
    function hideGridEditorController() {
      ...
      if (panel) panel.hidden = true;
      setGridEditorStatus("Choose a grid style for the stage.");
    }
    ...
    async function refreshVenueMapStateController() {
      try {
        const response = await fetchFn("/api/venues/" + (globalThis.VictoryStageVenue || {}).slug + "/map", {
          credentials: "include",
          cache: "no-store",
        });
        const payload = await response.json().catch(() => null);
    ...
    async function refreshVenueGridConfigController() {
      try {
        const response = await fetchFn("/api/venues/" + (globalThis.VictoryStageVenue || {}).slug + "/grid", {
          credentials: "include",
          cache: "no-store",
        });
        const payload = await response.json().catch(() => null);
    ...
    async function saveVenueMapController() {
      ...
      syncMapEditorWithState();
      void refreshVenueMapAssetsController().catch((error) => {
        console.warn("refreshVenueMapAssets after save failed", error);
      });
      livePreviewMapOnStage();
      return nextState;
    }
    ...
    function openMapEditor() {
      ...
      if (!canManageIndexCards(getCurrentRole())) {
        setStageStatus("Only producers and directors can add or replace the stage map.");
        return;
      }
      hideCardEditor();
      hideGridEditorController();
    ...
    function openGridEditor() {
      ...
      if (!panel) return;
      if (!canManageIndexCards(getCurrentRole())) {
        setStageStatus("Only producers and directors can configure the stage grid.");
        return;
      }
      hideCardEditor();
      hideMapEditorController();
