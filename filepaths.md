...
[packaging/podman/compose.yml](packaging/podman/compose.yml) defines `victory-postgres` and `victory-backend` (Kernel 96: the repo root's own docker-compose.yml, Grant's old bespoke production file, is retired — this is the one deployment path now, for production and self-hosters alike; `.env` comes from [packaging/podman/generate-env.sh](packaging/podman/generate-env.sh), which asks only lot name and Operator handle). Production layers [packaging/podman/compose.production.yml](packaging/podman/compose.production.yml) on top for the external `edge_net` network. TLS and static file serving are handled by the **shared `bread-caddy` container**, whose real config lives in `/opt/bread-exchange/` — not by this repo's [Caddyfile](Caddyfile). Migrations auto-apply at backend boot with a pre-apply dump into [backups/](backups). Runtime uploads live in [storage/](storage).

Deploy is `docker compose -f compose.yml -f compose.production.yml build backend && docker compose -f compose.yml -f compose.production.yml up -d backend` (run from `packaging/podman/`); confirm the migration lines in `docker logs victory-backend`.
