...
type OOCMessageRequest struct {
	SessionID string `json:"session_id"`
	ActorID   string `json:"actor_id"`
	Text      string `json:"text"`
}

// StoreOOCMessage records an out-of-character chat line. It is modeled
// directly on StoreChatMessage (chat.go) -- same authority gate, moment_id
// sequencing, and actions-table insert -- but stamps type "chat/ooc" so
// clients can filter it into an OOC lane separate from in-character chat.
func StoreOOCMessage(ctx context.Context, pool *pgxpool.Pool, req OOCMessageRequest) (*StoredAction, error) {
	...
	decision, err := CanAct(ctx, tx, req.ActorID, "chat/ooc", req.SessionID, ActionTarget{Kind: "session"})
	if err != nil {
		return nil, err
	}
	if !decision.Allowed {
	...
	if err := tx.QueryRow(ctx, `
		...
		VALUES ($1, $2, $3, 'chat/ooc', $4, $5, $6, $7, $8, TRUE)
		RETURNING id, ts
