...
func insertTestUser(t *testing.T, pool *pgxpool.Pool, handlePrefix string) string {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(bg, `DELETE FROM show_cohort_assignments WHERE user_id = $1 OR assigned_by_user_id = $1`, userID)
		_, _ = pool.Exec(bg, `DELETE FROM show_cohorts WHERE created_by_user_id = $1`, userID)
		...
		`, userID)
		_, _ = pool.Exec(bg, `DELETE FROM show_scene_placements WHERE created_by_user_id = $1`, userID)
		_, _ = pool.Exec(bg, `DELETE FROM scenes WHERE created_by_user_id = $1`, userID)
...
// showFixture builds Location -> Production -> Show Run -> Show, granting
// the director producer authority at the location. Reuses the fixed
// amurray-family location, matching scenes_test.go's productionFixture
// convention (a fresh DB has none of this runtime-shaped data).
func showFixture(t *testing.T, pool *pgxpool.Pool, directorUserID string) (showRunID, showID string) {
	t.Helper()
	ctx := context.Background()
	suffix := testSuffix(t)
...
// playerFixture creates a user, a My-People-style profile workbook (needed
// for AddRosterMember's profile-id resolution), a Character Card, and adds
// them to the Show Run as an active player with that Character selected.
func playerFixture(t *testing.T, pool *pgxpool.Pool, directorUserID, showRunID, locationID, handlePrefix string) (userID, characterCardID string) {
	...
	// Player-role roster membership goes through the two-punch ticket flow
	// (AddRosterMember rejects a direct "player" add with
	// player_requires_ticket) -- Director invites, then the player
	// self-accepts, matching tickets.InviteFromDirector/SecondPunch's real
	// production path.
...
// locationForShowRun resolves a Show Run's location, for callers that need
// it directly (e.g. playerFixture's character_cards.location_id).
func locationForShowRun(t *testing.T, pool *pgxpool.Pool, showRunID string) string {
	t.Helper()
	var locationID string
	if err := pool.QueryRow(context.Background(), `SELECT location_id::text FROM show_runs WHERE id = $1`, showRunID).Scan(&locationID); err != nil {
		t.Fatalf("resolve location for show run: %v", err)
	}
	return locationID
}
