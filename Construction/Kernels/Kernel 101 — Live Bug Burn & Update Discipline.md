...
| 101-07 | Raw internal error strings leak to the client (~21 sites in `ws.go`/`indexcard_http.go`/`director_console.go`) | OPEN | Bounded, mechanical fix |
...
| 101-12 | Dice-roll test's nil-pool mock is incomplete | OPEN | Confirmed test-harness artifact, not a production bug |
