...
| 101-20 | Follow-up to 101-19: after the cache-bust fix, Grant's own Catharsis account still showed "You are part of the Audience," the Cast/Director drawers and top bar hidden, while the Cartography toolbar (a Cast+ tool) stayed visible | **FIXED and deployed** | Confirmed via the live `/api/session/catharsis/join` response that Grant's account genuinely resolves `role: "cast"` — cartography was correct. The Audience banner/overlay is driven by a *separate* signal, `theater_context.kind` from `GET /api/world/catharsis` (`resolveTheaterContext` in `backend/internal/world/snapshot.go`), which only elevates a viewer above "audience" if they're a registered player in the *current live Show Run's* roster. `isBackstageRole` (its backstage-tier allowlist) already special-cased Crew for exactly this gap but never included Cast, so a Cast-approved account with no roster seat in the currently-live run fell all the way through to "audience." Grant's call: Cast approval alone should always mean "no audience view, ever" regardless of this run's roster. First attempt (adding "cast" directly to the shared `isBackstageRole`) was wrong and caught by a real test failure (`TestSnapshotWithholdsScopeMetadataFromPlayers`): that function also gates the milestone-gate bypass and, more seriously, `stageobjects`' hidden-object perception (`Viewer.Backstage`) — broadening it would have let any Cast-role account see Director-only hidden stage objects for shows they aren't even part of. `stageobjects/projection.go` has its own deliberately-separate duplicate of the same-looking function for that exact reason (Cast-vs-Audience there is resolved from the Show Run roster via `isShowCast`, not the role string) — reverted that file back to its original producer/director/operator/crew list untouched. Fixed correctly with a new, narrower `theaterContextBackstageTier` used only inside `resolveTheaterContext`, leaving `isBackstageRole` and its other three call sites (milestone gate, cohort-placement override, stage-object hidden-marker filter) unchanged. New test added to `snapshot_test.go` (`TestLoadVenueSnapshotTheaterContextParticipantAndAudienceMessages`'s cast case): confirmed it fails against the pre-fix code (`got audience`, wanted `backstage`) and passes with the fix. Full `go test -p 1 -count=1 ./...` run clean against a real local database (started a throwaway local `victory-postgres` for this, torn down after). Deployed via `docker compose up -d --build backend` (compiled Go change, not bind-mounted). |

| 101-21 | Follow-up to 101-20: Grant reported still not seeing Cast+ drawers/top bar in Catharsis even after 101-20 shipped | **FIXED, not yet deployed** | Root cause: `kernel93-audience-overlay.js`'s `applySnapshot` hides `#left-drawer`/`#right-drawer`/the drawer edge triggers/`#chat-panel`/`#top-bar` via inline `style.display = "none"` whenever `theater_context.kind === "audience"`, but the `kind !== "audience"` branch only ever set a code comment claiming to "leave the ordinary Cast/Director drawers alone" — it never actually restored that inline style. This module polls `GET /api/world/catharsis` independently every 10s (not tied to the main socket pipeline), so any single poll that ever observed `kind === "audience"` for a viewer — including via the exact 101-19/101-20 bug, or any other transient misresolution — permanently hid their Cast/Director UI for the rest of that browser tab's life, survivable only by a full page reload racing the next poll. Fixed by explicitly clearing the inline style (`style.removeProperty("display")`) on all the same elements in the non-audience branch, so the drawers/top bar self-heal on the very next 10s poll instead of requiring a reload. Bumped the file's own cache-bust tag (`?v=k101-21`) since Kernel 95's convention. `node --check` clean; this file has no test harness (plain browser-global script, matching every other stage-runtime file's existing coverage gap) — not run in a live browser this session. |

| 101-22 | Cast (confirmed via `/api/account/me`: role "cast", not operator/producer) sees the "Rehearsal available — open Stage Management" banner and can navigate into Stage Management, landing on an empty "No Show Runs yet" state | **FIXED, not yet deployed** | Real regression from 101-20: `renderRehearsalBanner` (`runtime.js`) gated the banner purely on `theater_context.kind === "backstage"`, which was a safe proxy for "Director/Producer/Operator/Crew" until 101-20 deliberately broadened that same `kind` value to also cover Cast (for an unrelated purpose — which drawer/banner experience Cast gets, not stage-management authority). The real data endpoints (`ListShowRunsVisibleToUser`, `handleListProductions`) already correctly return zero rows/403 to Cast, so no privileged data was ever exposed — but Cast could still navigate to the page and see a confusing empty shell, and got invited there by a banner that should never have shown for them. Fixed by checking the actual role (`producer`/`director`/`operator`/`crew`) directly instead of reusing `theater_context.kind`, decoupling the banner's authority check from 101-20's drawer-only broadening. `node --check` clean; not run in a live browser this session. |

| 101-23 | Root cause of the whole 101-19/101-20/101-22 saga, finally found: even after 101-20, Grant's Cast account (confirmed via `/api/session/catharsis/join` returning `role: "cast"`) still got `theater_context.kind: "audience"` from `/api/world/catharsis` on a genuinely fresh browser session (Firefox fully closed and reopened) — the Send Fanmail drawer stayed visible, Cast drawers stayed hidden | **FIXED, not yet deployed** | `resolveTheaterContext`'s `viewerRole` parameter never actually arrived as the string `"cast"` in the first place — it arrived as `"audience"`, mangled upstream by `participation.ResolveParticipationContext` (via `lookupVenueRole`/`LegacyLookupVenueRole`, the only role source for every `/api/world/*` snapshot). That resolver's Step 2 (`location_memberships`, the canonical account-level role table) only ever recognized `"producer"`/`"director"` in its switch statement — a location-level `"cast"` or `"crew"` role fell through every canonical step to Step 4's blanket "any active `location_memberships` row at all → audience" fallback. So 101-20's `theaterContextBackstageTier` fix, which correctly treats the literal string `"cast"` as backstage-tier, never had a chance to fire: the string was already rewritten to `"audience"` several layers upstream. Fixed by adding `"cast"`/`"crew"` cases to Step 2 (new `ViewerModeCast` constant; `"crew"` gets full `CanViewBackstage` matching the product model's "Crew always sees full backstage state," `"cast"` deliberately does NOT get `CanViewBackstage` — hidden-object perception stays gated on real Show Run roster membership via `stageobjects.isShowCast`, never on the bare location role, per the 101-20 near-miss). This also required guarding Step 3 (`show_run_roster_members`) so a location-level Crew member isn't downgraded to mere `"player"` if this specific show run's roster happens to list them as a player too — a real edge case this fix newly introduced, caught before shipping. New tests `TestCastLocationMembershipResolvesAsCastNotAudience` and `TestLocationCrewNotDowngradedByShowRunPlayerRole`: both confirmed failing against the pre-fix code (one by compile error — `ViewerModeCast` didn't exist — one by assertion, `got "player"` wanted `"crew"`) and passing after. Full `go test -p 1 -count=1 ./...` run clean against a real local database. |

| 101-24 | Three UI reports: (1) the tray "Always closed" open-mode option hides the drawer with no way to reopen it; (2) the floating "Back to Map" pill overlaps the header's title/status chips; (3) the right (Character/Notes) drawer's top edge is covered by the header when the header is pinned open | **FIXED, not yet deployed** | (1) `applyDrawerState`/`setDrawerHoverState` (`runtime.js`) forced the drawer permanently closed for `"always-closed"` with no code path back except an onboarding-only internal override never wired to any user control -- a real dead end once saved. Removed the option from both venues' Open mode `<select>`s, removed the now-dead branches, and added a load-time migration that coerces any already-saved `"always-closed"` value to `"hover"` so existing local prefs can't reproduce it. (2)+(3) both had the same root cause: `.drawer`/`.drawer-edge-trigger`'s `top` and the floating Back to Map button's `top` were static guesses (`66px`/`12px`) at the header's collapsed height, but the header's real rendered height varies (pinned/expanded is taller, and can wrap). Added a `ResizeObserver` on `#top-bar` that continuously tracks its actual height into a `--header-rendered-height` CSS variable (fires across the header's own open/close transition, not just the two endpoints), and switched all three static values to `calc(var(--header-rendered-height, <old-default>) + 8px)` -- exact clearance in every header state, on every venue that sets the variable (Catharsis, First Theater), with the old static behavior preserved as the fallback everywhere else (`back-to-map.js` is shared by three other venues that never set this variable). `node --check` clean on all three touched JS files; not run in a live browser this session. |
