...
- **eWrite skill-directory seed test can't reproduce a genuinely new catalogue entry.** `TestEnsureSkillDirectorySeedsAndIsIdempotent` (`backend/internal/ewrite/seed_skill_directory_dbtest_test.go`) asserts that `EnsureSkillDirectory` inserts a deliberately-fictional skill (`SKILL_TEST_NOT_IN_MANUSCRIPT`, chosen specifically to have no matching Core Rulebook section heading) alongside two real ones, but the fictional one never appears in the read-back afterward — the two real refs (already present from the database's own bootstrap) do. Confirmed unrelated to location resolution specifically: `EnsureSkillDirectory` (`backend/internal/ewrite/seed.go:404`) correctly resolved the real, fully-populated (~100 real skills) directory in every reproduction, ruling out the `access.DefaultLocationSlug()` env-var gap this same pass found and fixed for other tests via `dbtest.OpenTestPool`. Reproduced consistently across multiple genuinely fresh `victory_test` databases, so not test-state pollution either. Not investigated further: whether `EnsureSkillDirectory`'s title-matching path (the function is documented to link a catalogue entry to a Core Rulebook section by exact heading match, and separately claims an entry with no match "comes back unlinked rather than erroring") actually has a bug specifically in the no-match/unlinked insert path. Suggested classification: **UNKNOWN — requires targeted follow-up.**

- **Scene capture/promotion tests fail on layer state.** `TestUpdateCurrentScenePromotesShowLayerIntoBase` and `TestSaveArrangementAsNewSceneLeavesOriginalUnchanged` (`backend/internal/scenes/capture_test.go`) both fail asserting on layer/element state after a capture operation (one expects a promoted element's layer to read `"base"`, gets `"show"`; the other expects 2 copied base elements, gets 0). No reference to `DefaultLocationSlug`/`amurray-family`/`is_default` anywhere in the relevant files, so unlike the eWrite finding above this doesn't appear connected to this pass's own migration-architecture change at all — genuinely not investigated beyond confirming that non-connection. Reproduces consistently on a fresh `victory_test`. Suggested classification: **UNKNOWN — requires targeted follow-up.**
