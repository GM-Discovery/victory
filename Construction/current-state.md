...
- Current completed kernel: **Kernel 86A — Spatial Dice Landing & Explosion-Safe Projection** (individual dice now animate onto the actual active map and land at their own distinguishable, deterministic, map-relative coordinates — not Kernel 86's fixed screen-space "transient slot" — riding the same `stageCamera`/`worldLayer` pan-zoom transform every token/pin already uses; the preserved announcement banner is now sequenced behind a hard settle barrier so it never appears before every die has landed; pinned dice persist at their exact landed coordinates across reconnect. Full explosion-safety audit required by spec §2.3 found the default roll path was **already** safe — no auto-explosion exists anywhere in the current codebase's default path — so **no Kernel 86B was required** and the explosion-semantics Known Gap below is resolved, not just deferred)
- Completion date: **2026-08-11**
- Status: **PASS, deployed live, uncommitted** — no migration this kernel; the only backend change is a new test file (`backend/internal/dice/dice_test.go`), everything else is frontend. See `Construction/OperatorLogs/kernel-86A-reportback.md`.
