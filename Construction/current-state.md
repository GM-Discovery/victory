...
Kernel 85 (Socio Sustained Play), Kernel 86 (Theatrical Roll Projection), and Kernel 86A (Spatial Dice Landing & Explosion-Safe Projection) are all now complete, ahead of the sequence this file previously described. Kernel 86A's own spec named its purpose explicitly: it exists to make **Cartograph-style drawing** — a shared vector-drawing/layers/undo/export primitive toward an original or permission-safe map-making proof — safe to build. Both of Cartograph's former blockers are now cleared: dice land at real, deterministic, map-relative coordinates (not a screen-space slot), and the explosion-semantics question is resolved (audited safe, no engine change needed, no 86B required). Cartograph is not fully specified yet by design; the next kernel's own maker should audit current state first, and should read `Construction/Dice/spatial-dice-projection-contract.md`'s "Known limitation" section before relying on pinned-dice coordinates as a pixel-exact reference across every possible reconnect scenario.
