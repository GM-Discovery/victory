...
- any keyboard-reachable entry point for the Presence Tray's context menu (currently right-click only, a named accessibility gap — see `Construction/Domains/Storyboards/storyboards-accessibility.md`);
...
| Storyboards WS pump reusing a handshake-scoped context for a connection's whole life | **Repaired (Kernel 84)** | Was a real bug (late `watch_board`/board-switch operations silently failed past ~5s), not a documentation gap — see `Construction/Domains/Operations/websocket-context-lifecycle.md` |
...
Full ledger and evidence: `Construction/OperatorLogs/kernel-90-reportback.md`; operator guide: `Construction/Domains/Operations/stage-object-visibility.md`.
...
**Superseded.** Kernel 85 shipped, as did 86, 86A, 87, 88 and 89. The immediate next action is Grant's own live walkthrough of Kernel 89's Training Arena sequence (`Construction/Domains/Operations/director-prepared-play.md` §8), after which **Kernel 90** should take on the visibility/reveal-state reconciliation recorded in §17's 2026-08-17 entry — which is where the original text below belongs as history:
