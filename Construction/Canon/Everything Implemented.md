...
- Evidence: migration `047_kernel71_roster_character_selection.sql`, `backend/internal/participation/`, `showruns/roster.go`, and the Kernel 71 participation model in `Construction/Canon/current-state.md`.
...
- Evidence: `backend/internal/access/`, `participation/`, `shows/authority.go`, `scenes/authority.go`, `cues/authority.go`, action authority tests, and `Construction/Domains/Security/kernel-76-*` matrices.
...
- `Construction/Canon/current-state.md` still says visual composition/capture is not implemented, but that conflicts with this current code/migration/routes/tests and is stale. Kernel 85’s browser run did not separately click the two capture buttons, so their UI wiring has Go-test rather than browser proof.
...
- Evidence: migration `089`, `backend/internal/ewrite/`, link UI surfaces, `Construction/Domains/eWrite/ewrite-import-export.md`, Kernel 79 Goal C&E reportback.
...
- Evidence: `Construction/Domains/Operations/victory-backup-runbook.md`, `victory-restore-runbook.md`, Kernel 77 proof, `identity/backup_status.go`, deployment/fresh-install scripts.
...
- Evidence: `backend/internal/migrate/`, `backend/migrations/`, `Construction/Process/deployment/fresh-install.md`, `scripts/smoke/fresh-install.sh`, `main.go`.
...
- Evidence: `backend/internal/ratelimit/`, `network/kernel77_*`, `identity/kernel76_auth_closure_test.go`, `Construction/Domains/Security/`, eWrite/image-visibility migration `086`, Kernel 76–77 reports.
