...
Generic, reusable Group Leader / Current Turn state for any Victory venue where people work or play together live. This document is the contract a future venue reads before opting in — it does not itself specify Storyboards' particular wiring (see `Construction/Domains/Storyboards/session-state-integration-seam.md` for that).
