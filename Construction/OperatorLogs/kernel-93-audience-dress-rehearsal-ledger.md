...
**Status:** Setup complete, acceptance gate reached. Pass A in progress (started 2026-08-22) — see §5 for the full list of bugs found and fixed live, one open unresolved risk (A14), and two features investigated mid-pass (A18–A20).
...
Pass A in progress (2026-08-22 through 2026-08-23), live with Grant + a disposable second Audience account (`test-user-1`, direct DB session insert per the field guide's Two-Browser technique). Entries below are what actually surfaced testing the "Scene transition" and general stage-sync items on Pass A's checklist — most of the pass turned out to be blocked on infrastructure that had never actually been exercised live before, not on Audience-experience polish. Table is a summary; see chat history for full root-cause detail on each.
...
| A13 | A | Using Scene Configuration for live-improv testing was found to also change what a brand-new, characterless cast member sees landing in Catharsis (Kernel 74's local-projection protection only activates *after* a Player finishes Ra's dialogue — before that, everyone shares the one `current_show_scene_placement_id`) | Risk, disclosed | Fix — new sub-kernel | Built **Configurator Mode**: a private draft workspace reusing every existing token/card/map/grid tool unchanged (interception at `sendAction()` and at a wrapped `fetch` injected into `editors.js`), so building/switching arrangements never broadcasts to Audience or touches the shared "Ungrouped" projection until an explicit Activate. Added a "New Scene" button (Configurator Mode previously could only open an *existing* Scene's draft) | Built, **not yet verified live** | `frontend/lib/stage-runtime/runtime.js`, `kernel85-cohort-tools.js` |
| A14 | A | **Open, unresolved:** the original tutorial Scene's stored composition (Kessa, the door) may have been overwritten by an "Update Current Scene" click made before A10/A11 were understood, while that Scene still held the show-wide pointer | Risk | **Needs verification** | Grant believes it's recoverable by hand if so; not yet confirmed either way | **Open — verify by Activating the original placement (`70f0d3bb-f097-4cc9-9953-3f06b95d7cda`) and confirming Kessa/the door render correctly** | — |
| A15 | A | Kessa (a scene-composition element, not a real live element) cannot be removed via the normal stage context menu | Bug, minor | Not fixed | Likely just missing "remove" wiring for scene-composition-kind elements specifically; Grant has a workaround (hide via visibility control) | **Deferred** | — |
...
| A18 | A | Reactions absent for both Audience and Director | Known gap (see §0) | Deferred | Matches this ledger's own pre-Pass-A note: `react/emote` fully wired server-side, zero frontend UI, never built. Confirmed independently while scoping a chat replacement — part of the Kernel 1 "Audience Exchange" primitives (reactions + note cards) Grant wants ported into Catharsis, not started | **Not started** | — |
| A19 | A | Chat visible to Audience with no Director-controlled visibility toggle | Design gap | Fix — planned | No such toggle exists anywhere in the code (confirmed, not assumed); Grant wants chat replaced with the Kernel 1 note-card + reactions primitives instead of gated | **Not started** | — |
| A20 | A | Merchant "expose" flow confusing / didn't result in a visible, clickable merchant | Investigation pending | — | — | **Investigating** | — |
