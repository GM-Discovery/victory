...
> **Note on the gap:** this log skips Kernels 85–88C. Those passes recorded
> themselves in `Construction/OperatorLogs/kernel-85-reportback.md` through
> `kernel-88-reportback.md` (which also carries 88A/88B/88C) and did not
> write here. Those reportbacks are authoritative for that span.
...
- The Director's live surface was **regrouped rather than extended**. Kernel 85/88 had four flat top-level buttons and this kernel adds six operations; ten buttons would have been the button wall the spec forbids. Now: one **Director Tools ▾** selector holding seven families, plus **Send Aftercare** kept top-level on purpose. `kernel85-cohort-tools.js` stands down when it sees the new owner flag, so deleting the new file restores the old toolbar rather than leaving a Director with nothing.
- Stage and token right-click menus gained one nested "Director Tools ▸" entry that discloses its children on selection. `logic.js` stays pure (a `submenu` is plain data); the unit test asserts the children do **not** also appear at top level, since otherwise nesting buys nothing.
- Theatrical announcements ship as a ten-style palette defined **once**, on the server, in a pure leaf package — served to the client rather than duplicated there, so a style cannot mean one thing to the sender and another to the viewer. Each style is distinguished by glyph, frame shape, entrance motion, emphasis **and** its own name printed on the banner; a Go test fails if two styles ever collapse onto the same non-colour fingerprint, making the accessibility rule machine-checked. Delivery reuses Kernel 86's Stage Effect registry and `rollaudience` audience resolution — no new table, no migration, and pin/dismiss for free.
- **Victory still never decides what a roll meant.** There is no code path anywhere from a die to an announcement, and the acceptance proof asserts it as a property: the payload a Player receives contains only `style` and `text`, so there is nothing in it Victory could have inferred meaning from.
- Kernel 73's recorded gap — "a Director cannot edit Kessa's dialogue without a new migration" — is closed. Merchant authoring writes the same two canonical tables `LoadPacketBySlug` already reads, with stock chosen by id from the one 145-item equipment corpus; a foreign equipment id is refused, not created. Merchants can now be exposed to a single **Cohort**, enforced inside `ResolveEligibleContext` — the one Player-eligibility gate — so guessing an interaction id is not a way around it.
- **Send Aftercare** is delivery only: it opens each Player's own Kernel 75 form from the same endpoint the tutorial completion uses, writes no `aftercare_*` row, targets per-user (never a session broadcast), and reports offline recipients honestly. `/showtime end` still does not touch it and deliberately never will.
- **One criterion came back PARTIAL, found by trying to prove it rather than asserting it:** announcements in the **First Theater**. The modules are loaded there and the server handler is venue-agnostic, but `access.ResolveVisibleVenues` has zero `first-theater` branches, so no ordinary user can open that venue's socket at all — pre-existing, and not worth widening venue visibility to paper over. Harness kept at `scripts/smoke/kernel89-first-theater-announcement.js`, which preflights and reports the blocker.
- **Russel was reused, not built.** He is already published in Victory's own Library (Niava Setting Supplement, Chapter V, anchor `russel-master-at-arms`), so this kernel created no NPC record — building one would have been precisely the "contradictory second version" the spec forbids. No Training Arena Scene was seeded either: seeding it in a migration would have disproven the thing the kernel exists to prove.
...
- `participant_interactions.enabled` keeps its distinct meaning as a global authoring kill-switch; a new Show-scoped `interaction_enabled` dimension is ANDed with it rather than replacing it, so a Director disabling an interaction mid-Show cannot disable it in the tutorial or another Show.
- Scene-authored composition elements (Kessa's stall and its kin) had **no visibility control of any kind** before this kernel. They now get the same grouped Visibility/Interaction context-menu families as live warehouse tokens.
- **A real pre-existing bug was found, not fixed:** the live-venue role lookup resolves every Show Run roster Player to `"audience"` (it calls the participation resolver with no Show Run hint, so the roster step never runs). Kernel 90's own Cast/Audience scoping works around this correctly by deriving Cast from Show Run roster participation directly, proven by a dedicated test — but the underlying resolver bug is still live for anything else that trusts its role string on that path.
- **Acceptance evidence: 52/52**, real compiled backend, real HTTP, **five real viewers** (Director, two Cohort-assigned Players, one Ungrouped Player, one genuine Audience-tier viewer) each reading their own live snapshot — no mocked projections anywhere. Covers all five scope kinds including two grants on one object at once, the disabled-≠-hidden/hidden-≠-disabled independence with the invoke-time refusal proven, persistence across repeated fresh reads, the map explicitly refused against a real row, and a nine-case authority matrix.
- **Browser proof is a genuine, honestly-reported gap**: this host's headless Chromium cannot sustain a live PixiJS WebGL render — every software-GL configuration tried crashed the renderer (confirmed via `dmesg` as a real GPU-process trap, not memory pressure), and disabling WebGL leaves this Pixi bundle unable to initialize a canvas at all. The scope panel was proven live against real Cohort/Character data before the environment gave out; the menu-nesting and dimmed/dashed hidden-object treatment are covered by frontend unit tests and code review rather than a rendered screenshot. The proof script is left correctly configured for a healthy host with the failure mode documented in its own header.
...
**Status: PASS for confirmed scope, deployed live 2026-08-18.** Migrations 107 and 108 both applied cleanly at boot with pre-apply backups; frontend served directly from `/opt/victory/frontend` via the shared Caddy bind mount, so no build/restart step for JS/CSS/HTML changes. Full ledger: `Construction/OperatorLogs/kernel-91-reportback.md`.

- Built one shared, reusable guided-tour engine (`frontend/lib/tour-engine.js`) plus a new `backend/internal/tour/` package, replacing the old localStorage-only "Welcome to Victory" map modal with a server-persisted, role-aware system. Scope confirmed with Grant before implementation: full engine/schema/API for every role, but authored step content only for the mandatory campus tour (Audition Hall + Trailer), the skippable campus continuation (Catharsis + Greenroom), the Catharsis Cast venue tour, and the Director's Chair role-overlay tour — the spec's own §42-43 acceptance proof (a Cast member who becomes Director gets the toolbox tour without replaying the Cast one). Producer/Crew/Operator/Audience have working eligibility plumbing but no authored content yet, deferred per the spec's own §50-52 boundaries.
- Two migrations: 107 (`tour_completions`, the append-once completed/skipped record) and 108 (`tour_progress`, a resumable cursor added mid-kernel once a real bug required it — see below). Both use a `CREATE UNIQUE INDEX` over `COALESCE`-wrapped nullable columns rather than a table-level `UNIQUE(...)`, which Postgres rejects for expressions — caught by the migration itself failing against the real test database on first apply, not by review.
- **Three real bugs found in production by Grant, not in review, all fixed same-day:** (1) the dimming overlay was a single full-viewport div that physically intercepted every click itself, regardless of any click-gating logic downstream — "I can't click anything on the map." (2) the position-tracking `MutationObserver` watched the same DOM attributes it was writing into, triggering an infinite self-loop — surfaced as a uBlock Origin "page is slowing down your browser" warning. (3) a click-gated step whose target causes real navigation (the Audition Hall/Trailer pins) raced an ordinary `fetch()` against the page unload and lost, so progress was never persisted and the mandatory tour restarted from step 0 on every return to the map — "stuck in a loop." Fixed with four dimming rectangles that leave the target structurally clickable (not a synthetic re-dispatch), an observer that no longer watches its own writes, and `navigator.sendBeacon` (browser-guaranteed delivery across page unload) backed by the new `tour_progress` resumable cursor. All three recorded as reusable lessons in `operator-notes.md`.
- 13 new backend tests (idempotency, per-user/venue scoping, role gating, Operator bypass via `IsOperatorUser`, mandatory-skip rejection, forged-`user_id` rejection, progress-resume slicing, completion clearing the stale cursor); full Go suite green, `gofmt` clean. No scripted Playwright proof committed this pass — the core acceptance claims were exercised through real production usage instead, which is in fact how the three bugs above were found.

### Next recommended step
A scripted Playwright proof of the spec's §39-43 required scenarios, particularly the Director role-transition acceptance case (grant Director access mid-session via a fixture, confirm the toolbox tour becomes eligible without replaying the Cast tour) — exercised live but not yet by an automated, repeatable check. After that, authoring Producer/Crew/Operator/Audience tour content is a small, mechanical follow-up given the eligibility plumbing already in place.

## Kernel 92 — Showtime Composition & Showing Scheduler (2026-08-19)

**Status: PASS for confirmed scope, deployed live 2026-08-19.** Migration 109 applied cleanly at boot with a pre-apply backup. Full ledger: `Construction/OperatorLogs/kernel-92-reportback.md`.

- Composed scheduling, live-start, and live-end into one Director flow (**Showtime button → pick/schedule a Showing → SHOWTIME**) backed by one server-side domain operation (`backend/internal/showtime.Start/Status/End/Preflight`) shared identically by the GUI and the pre-existing `/showtime` command. Central finding from the required pre-implementation audit: **no new "Showing" table was needed** — the existing `shows` table (Kernel 67) already models a scheduled performance instance (timestamp, short code, persistent current-Scene pointer) and a Show Run can already own many Shows; the only real gap was a Director-facing nickname (migration 109), and the rest is additive extension of the pre-existing `showtime` package rather than a parallel system.
- New `Preflight` operation gives a real blockers-vs-warnings split — incomplete Characters, an empty ticket count, and a not-ready chat bridge are warnings that never block Showtime; a busy venue or no derivable venue are true blockers that disable the button. Start/End are now idempotent (`AlreadyLive`/`AlreadyEnded`, no duplicate Session ever created by a repeated press), and Start now actually connects the Discord chat-bridge thread (previously hardcoded off) as a non-blocking step. "House Mic" relabeled to "Chat Bridge" everywhere in user-facing text; the `/mic` command name and Discord slash command themselves left unchanged for compatibility.
- **Design finding, not a bug:** the Kernel 89 in-stage Director toolbar cannot host a "begin the show" button — it only renders once a live session snapshot already has a `show_id`, which is exactly the state Showtime is meant to create, not require. The new button instead lives on the Director's Chair page, the one venue-independent Director surface.
- **A real URL collision surfaced at boot, not in review:** the natural `/api/showings` path was already registered by the unrelated pre-existing Kernel 22 `showings` package (live audience-visibility review) — Go's `net/http` mux panics on conflicting route registration, caught on the first `docker compose up -d backend` of this kernel. New endpoints mounted at `/api/showtime/showings` instead.
- **A shared, long-lived test database silently fed one kernel's test assertions from a prior kernel's leftover fixture rows**, because `producer` role at a shared location grants manage-authority over *every* Show Run at that location, not just ones a given test created — fixed by scoping assertions to the test's own Show Run ID rather than assuming isolation; a related wall-clock-dependent Today-vs-Upcoming bucketing flake was also found and fixed the same way. Both recorded in `operator-notes.md`.
- 16 new backend tests (nickname validation, Today/Upcoming/Historical bucketing across every boundary and alphabetical-within-band, Show-Run-scoped visibility, live-session-derived liveness, start-by-ID equals start-by-code, double-Start/double-End idempotency, Preflight's blocker/warning split, Preflight's dry-run venue derivation never mutating state, and End's Aftercare-eligible count); full Go suite green. Real Playwright browser proof against production (disposable fixture-row Director account) — 7/7 checks passed for scheduling, browsing, and Preflight's real blocker output; deliberately stopped short of pressing GO TO SHOWTIME for real to avoid writing live Session state onto a real production venue.

### Next recommended step
A scripted Playwright proof of the true live-start/live-end path against a genuinely disposable venue/Scene fixture (not a real production venue) — the one claim in this kernel proven by Go tests over a real database but not yet by a scripted browser click.
