...
- Modified: `backend/cmd/victory/main.go` (route registration, Courtyard bootstrap call), `backend/internal/access/kernel16_venue_bootstrap.go` (fresh-install capability flag), `backend/internal/network/hub.go` (`BroadcastToSessionUser`), `frontend/lib/stage-runtime/runtime.js` (participant-interaction button rendering), both venue `index.html` (new script tags), `frontend/venues/show-runs/show.html` (Director authoring panel), `frontend/venues/greenroom/index.html` (Inventory nav button), `Construction/Canon/current-state.md`, `Construction/Canon/Dictionary.txt`, `Construction/Canon/roadmaps/victory-track-roadmaps-v1.md`, `Construction/OperatorLogs/operator-log.md`.

## Addendum (2026-07-20) — Canonical Socio Equipment Catalog

The operator supplied the real Socio-: Stories of Us equipment catalog, replacing the 5 invented placeholder items. Confirmed with the operator before implementation: new fields are **reference-only** (displayed, never computed against, deducted, or used to gate a purchase) — this does not reopen the locked "no automatic prices/currency" exclusion, since nothing server-side enforces or calculates against them.

**Schema** (`062_kernel73_equipment_catalog_fields.sql`, additive): `equipment_items` gains `category` (coarse grouping for shop-UI display), `cost_credits` (nullable numeric, the catalog's in-game "Cost" column only — "Cost at 1 Wealth" and "USD Cost" were deliberately excluded as GM-side derivation references, not shop-facing data), and `stats_json` (a flexible per-category bag — damage/damage_reduction/tags/special/effect — since a weapon's damage die and a mount's travel bonus share no common column shape).

**Data** (`063_kernel73_socio_equipment_catalog_seed.sql`): all 145 catalog items transcribed as `equipment_items` rows (reusable reference data for future merchants, not Kessa-exclusive), deactivating (not deleting) the 5 Kernel 73 placeholders. Kessa's own shop stock is a curated 19-item starter-appropriate subset (basics, low-tier weapons/armor, tools, survival gear) — operator-approved before writing the migration — not the full catalog; endgame gear like Enchanted Plate or a War Horse exists in the catalog but isn't purchasable from a Courtyard onboarding vendor.

**Go**: `EquipmentItem`/`EquipmentItemInput`/`EquipmentItemPatch` and the equipment editor's HTTP handlers extended for the three new fields; existing purchase/inventory code paths untouched (no behavior change to the transaction/idempotency logic Kernel 73 already proved).

**Verification**: new `TestSocioEquipmentCatalogSeed` (item count, a sample weapon's category/cost/damage stats, a fractional-cost consumable, Kessa's stock being a proper subset not the full catalog, an excluded endgame item's absence from her stock). `TestKessaGoldenPath` updated to reference the new catalog's slugs. Full suite + `alpha-gate.sh` green; a dedicated fresh-database scratch boot independently confirmed 145 active items and Kessa's 19-item stock on a from-empty install (no Go-bootstrap ordering gap this time — unlike the Courtyard Scene, this migration only depends on the `locations` row, which migrations create directly, not a Go-seeded venue).

**Deployed live 2026-07-20**: `2 pending → pre-apply backup → both applied (<70ms) → schema current`; live content spot-checked and matches exactly.
