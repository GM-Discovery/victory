...
- **Process documents updated in place** (not duplicated): `Construction/kernel-maker-field-guide.md` (was frozen at ~Kernel 33, now points to the new archive and current architecture), `Construction/reportbacktemplate.txt` (6 new sections modeled on tonight's own ledgers), `Construction/current-state.md` and `Construction/roadmap.md` (additive corrections only, pointing at the new Kernel Debt Ledger rather than being rewritten).
