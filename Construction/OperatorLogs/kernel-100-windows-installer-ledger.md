...
4. **Third-party notice changes:** `packaging/windows/VictoryLauncher/THIRD_PARTY_NOTICES.md` (ships with the app) now covers every Windows-bundled component with pinned versions; `Construction/Process/vendor-acknowledgements.md` updated to point at it as canonical rather than duplicate it.
